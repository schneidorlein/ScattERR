...
class MotorControl(object):
    ...
    def __init__(self, GUI=None):
        ...
        # Variables
        self.pos = []  # Unit: mm
        self.Step2MM = np.array([1E4, 1E4, 1E5])
...
#            self.WatchMovement()
    ...
    def get_Position(self):
        ...
        y = float(self.serial_query(self.slaves[0], 1, 'CNT'))/self.Step2MM
        x = float(self.serial_query(self.slaves[1],  1, 'CNT'))/self.Step2MM
        z = float(self.serial_query(self.slaves[2],  1, 'CNT'))/self.Step2MM
...
if __name__=="__main__":
    Motor = MotorControl()
    Motor.InitializeCOM(Motor.portlist[1])
    Motor.InitMotor()
